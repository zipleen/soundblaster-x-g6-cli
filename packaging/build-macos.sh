...
#
# Usage:   packaging/build-macos.sh [--skip-dmg]
#
set -euo pipefail
...
command -v python3.12 >/dev/null || die "python3.12 not found. Install it (e.g. 'brew install python@3.12')."
xcode-select -p >/dev/null 2>&1 || die "Xcode command line tools missing. Run: xcode-select --install"
ok "macOS, python3.12, command line tools"
...
LIBUSB_SRC="$(python3.12 -c 'import os,sys; print(os.path.realpath(sys.argv[1]))' "$LIBUSB_SRC")"
ok "libusb: $LIBUSB_SRC"

# ── 2. Build toolchain ───────────────────────────────────────────────────────
info "Preparing build environment ($BUILD_VENV)"
[[ -d "$BUILD_VENV" ]] || python3.12 -m venv "$BUILD_VENV"
...
# ── 5. Package ───────────────────────────────────────────────────────────────
if [[ "$SKIP_DMG" == "--skip-dmg" ]]; then
    info "Skipping .dmg (--skip-dmg)"
else
    info "Packaging .dmg"
    # --adhoc-sign: signed locally, not notarised. Enough to run on this Mac and
    # on any Mac the file is copied to directly. See packaging/README.md.
    "$BUILD_VENV/bin/briefcase" package macOS app --adhoc-sign
...
packaging/verify-macos-app.sh "$APP_PATH"
